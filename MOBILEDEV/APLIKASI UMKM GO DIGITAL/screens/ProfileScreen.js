    ...
    <View style={styles.infoRow}>
      <View style={styles.infoLeft}>
        <View style={styles.infoIcon}>
          <Ionicons name={icon} size={18} color="#EE4D2D" />
        </View>
  ...
  const handleClearData = () => {
    Alert.alert('Reset Data', 'Semua data produk dan transaksi akan dihapus. Lanjutkan?', [
      { text: 'Batal', style: 'cancel' },
      {
        ...
        onPress: async () => {
          await Storage.multiRemove(['umkm_products', 'umkm_transactions', 'umkm_expenses', 'umkm_categories']);
          onRefresh();
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 30 }}>
        <View style={styles.statsRow}>
          <StatBox value={totalTx} label="Transaksi" color="#EE4D2D" />
...
const styles = StyleSheet.create({
  ...
  categoryText: { fontSize: 12, fontWeight: '700', color: '#EE4D2D' },
  ...
  editBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: '#EE4D2D', borderRadius: 16, height: 54, marginTop: 20, elevation: 4 },
